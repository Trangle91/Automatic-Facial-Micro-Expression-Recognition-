...
####################################################################################
def main():
    start_epoch,max_epoches,best_acc=0,100,0
    Training_loss=[]
    Validation_loss=[]
    Accuracy = []
    model=VGG_model.vgg19_bn(pretrained=False,num_classes=7)
    ...
    ###################### Resume checkpoint  ##################################
    try:
        state=torch.load('./Models/VGG_19/Last_Model.pth') 
    ...
    ################################################################################    
    train_data=Database('./Database/Face_expression/Train/')
    dev_data=Database('./Databases/Face_expression/Eval/')
    ...
    for epoch in range(start_epoch,max_epoches):
        ...
        if (np.mean(accuracy)>best_acc):
            ...
            torch.save(state,'./Models/VGG_19/Best_Model.pth')
        ...
        torch.save(state,'./Models/VGG_19/Last_Model.pth')
