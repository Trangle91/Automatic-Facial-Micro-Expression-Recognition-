...
def main():
    ...
    Validation_loss=[]
    Accuracy = []
    model=VGG_model.vgg16_bn(pretrained=False,num_classes=7)
    ...
    ###################### Resume checkpoint  ##################################
    try:
        state=torch.load('./Models/VGG_16/Last_Model.pth') 
    ...
    dev_data=Database('./Database/Face_expression/Eval/')
    ...
    for epoch in range(start_epoch,max_epoches):
        ...
        if (np.mean(accuracy)>best_acc):
            ...
            torch.save(state,'./Models/VGG_16/Best_Model.pth')
        ...
        torch.save(state,'./Models/VGG_16/Last_Model.pth')
