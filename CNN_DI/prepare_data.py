...
def process_img(folder_in , img_name  , folder_out , aug = 0 ):
    ...
    out_img_path = os.path.join(folder_out , img_name)
    ...
    if (aug == 1):
        ...
        new_img = new_img.astype(int)
        out_img_path = os.path.join(folder_out ,  'flip_' + img_name)
        cv2.imwrite(out_img_path , new_img  )
...
class SDI_MER():
    def __init__(self , raw_data_folder , working_folder , facial_region = 0, test_ratio = 0.2 , num_cross = 5 ):
...
def main():
    dat = SDI_MER('SAAM_train_data', 'saam_data')
